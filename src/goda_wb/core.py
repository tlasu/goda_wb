...
def etai(i, dl0, etl0):
    if(i<=1):
        ...
        etl0[i]=etl0[i-1]+(etl0[i-1]-etl0[i-2])*(dl0[i]-dl0[i-1])/(dl0[i-1]-dl0[i-2])
        return etl0
...
def setup(etl0:list[float],
          dl0:list[float],
          h0l0:float,
          p:list[float],
          xp:list[float],
          i:int,
          h2l0:list[float]) -> tuple[list[float], list[float]]:
    #mmax=100
    #mmax=len(p)
    mp=len(p)
...
def out(h0l0:float,
    ...
    mp = 51
    nwave = 7    
    hnh0 =[0.  for i in range(nwave)]
    hnd  =[0.  for i in range(nwave)]
...
def prob(x1:float,
         ...
         pxi:float,
         xp:list[float],
         p:list[float]) -> tuple[list[float], list[float]]:
...
class cla_shoal:
    ...
    def calc(self):
        ...
        #  find out h30
        #
        while (np.abs((self.dl-self.dl1)/self.dl)>1.e-6):
            self.dl = self.dl1
            self.n = self.n+1
            sh4 = np.sinh(2.0*pi2*self.dl)
            th2 = np.tanh(pi2*self.dl)
            ch2 = np.cosh(pi2*self.dl)
            f = (self.dl*th2)**2-self.alfa/np.sqrt( (1.0+2.0*pi2*self.dl/sh4)*th2)
            df=2.0*self.dl*th2**2+2.0*self.dl**2*pi2/ch2**2+ \
                pi2*self.alfa*np.sqrt(sh4)*th2*(2.0-pi2*self.dl*th2) \
...
def shoal(dl0:float, h0l0:float) -> float:
    ...
    while (np.abs((dl-dl1)/dl)>1.e-6):
        ...
        df=2.0*dl*th2**2+2.0*dl**2*pi2/ch2**2+ \
           pi2*alfa*np.sqrt(sh4)*th2*(2.0-pi2*dl*th2) \
    ...
    c50=aks50*d50l0**1.5*(np.sqrt(pi2*h0l0*aks50)-2.0*np.sqrt(3.0)*d50l0)
...
def cal_surf_goda(tant:float, h0l0:float, dl0:list[float]|None=None) -> pd.DataFrame:
    """
    h0l0:波形勾配
    tant:海底勾配
    dl0:水深波長比
    ...
    if dl0 is None:
        dl0=[1.0, 0.9, 0.8, 0.7, 0.6, 0.5,
            0.45, 0.425, 0.4, 0.38, 0.36, 0.34, 0.32, 0.3, 0.28, 0.26,0.24,0.22,
            0.20, 0.19, 0.18, 0.17, 0.16, 0.15, 0.14, 0.13, 0.12,
            0.11, 0.10, 0.09, 0.08, 0.075, 0.07, 0.065, 0.06, 0.055, 0.0525, 0.05,
            0.0475, 0.0450, 0.0425, 0.0400, 0.0375, 0.0350, 0.0325, 0.0300,
            0.0275, 0.0250, 0.0225, 0.0200, 0.0190, 0.0180, 0.0170,
            0.0160, 0.0150, 0.0140, 0.0130, 0.0125, 0.0120, 0.0110, 0.0100, 0.009,
            0.00875, 0.0085, 0.00825, 0.008, 0.00775, 0.0075, 0.00725, 0.007, 0.00675,
            0.00650, 0.00625, 0.00600, 0.00575, 0.00550, 0.00525, 0.00500,
            0.00475, 0.00450, 0.00425, 0.00400, 0.00375, 0.00350, 0.00325,
            0.00300, 0.00275, 0.00250, 0.00225, 0.00200, 0.00190, 0.00180, 0.00170, 0.00160, 0.00150, 0.00140, 0.00130, 0.00125, 0.00120, 0.00110, 0.00100, 0.00090, 0.00080, 
            0.00075, 0.00065, 0.00055, 0.00045, 0.00035, 0.00025, 0.00015, 0.00005, 0.00001, 0.000000001]
    iflag=[1 for i in np.arange(len(dl0))]
    ipoint=len(dl0)
    etl0=[0.  for i in range(ipoint)]
    h2l0=[0.  for i in range(ipoint)]

    pp =[0.  for i in range(ipoint)]

    qq1=[0.  for i in range(ipoint)]
    qq2=[0.  for i in range(ipoint)]
    qq3=[0.  for i in range(ipoint)]
    ...
    for m in range(0,ipoint):
        ...
        p_.append(p)
        aks_.append(aks)
    df = pd.DataFrame(hnh0,columns=["H1_1000","H1_250","H1_100","H1_50","H1_10","H1_5","H1_3"])
    df["dh0"]=dh0
    df["dl0"]=dl0
    df["etal0"]=etl0
    df["aks"]=aks_
