...
def draw_on_image_goda():
    # 画像を読み込み背景として表示（ピクセル座標に合わせてextentを設定）
    img = mpimg.imread("../surf_2.png")
    img_height, img_width = img.shape[:2]
    img = img[:, img_width // 2 :]
    rotation_angle = 0.5  # 画像を少し回転（度数法）
    ...
    x0, y0 = 100, -140  # 画像上でグラフを始める位置（左上）
    box_width, box_height = 860, 1800  # グラフ領域の幅と高さ
    ...
    bbox_fig = TransformedBbox(
        bbox_disp, fig.transFigure.inverted()
    )  # 画面座標 → Figure 座標
